...
export const ServerRoute = createServerFileRoute("/upload").methods({
  POST: async ({ request }) => {
    try {
      ...
      if (!file || !(file instanceof Blob)) {
        throw new Error("Aucun fichier uploadé ou le fichier n'est pas valide");
      }
      if(!formDataQuality) {
      ...
      try {
        ...
        if (e instanceof Error) {
          ...
          if (msg === "Input image exceeds pixel limit")
            throw new Error("L'image dépasse la limite de pixels autorisée.");
